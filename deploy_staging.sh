...
echo "========================================="
echo "FlexyRide Ultimate Production Deployment"
echo "========================================="
echo ""
...
DOMAIN="flexyridegh.com"  # Update this if your domain is different (e.g. .gh)
USER="patmac"
GROUP="www-data"
APP_REPLICAS=2
DB_REPLICAS=0
...
pip install gunicorn daphne uvicorn[standard] django-redis django-tinymce google-auth requests
...
sudo -u postgres /usr/lib/postgresql/18/bin/psql -c "CREATE DATABASE flexyride_staging_db;" || echo "Database already exists"
sudo -u postgres /usr/lib/postgresql/18/bin/psql -c "CREATE USER $USER WITH PASSWORD 'qwerty%asdf';" || echo "User already exists"
sudo -u postgres /usr/lib/postgresql/18/bin/psql -c "GRANT ALL PRIVILEGES ON DATABASE flexyride_staging_db TO $USER;"
# Fix for PostgreSQL 15+: Grant creation rights on public schema
sudo -u postgres /usr/lib/postgresql/18/bin/psql -d flexyride_staging_db -c "ALTER SCHEMA public OWNER TO $USER;"
sudo -u postgres /usr/lib/postgresql/18/bin/psql -d flexyride_staging_db -c "CREATE EXTENSION IF NOT EXISTS postgis;"
sudo -u postgres /usr/lib/postgresql/18/bin/psql -d flexyride_staging_db -c "CREATE EXTENSION IF NOT EXISTS postgis_topology;"
...
command=$VENV_DIR/bin/daphne -b 127.0.0.1 -p $PORT --access-log - --proxy-headers flexy_backend.asgi:application
directory=$APP_DIR
user=$USER
autostart=true
autorestart=true
...
command=$VENV_DIR/bin/celery -A flexy_backend beat -l info
directory=$APP_DIR
user=$USER
autostart=true
autorestart=true
...
rm -f /etc/nginx/sites-enabled/flexyridebackend
...
echo "========================================="
echo ""
echo "Application: FlexyRide (flexyride_backend)"
...
echo ""
echo "🚀 FlexyRide is production-ready!"
echo ""
